...
class Squats(ExerciseBase):
    def process(self, landmarks):
        ...
        knee_angle_l = calculate_angle([hip_l.x, hip_l.y], [knee_l.x, knee_l.y], [ankle_l.x, ankle_l.y])
        knee_angle_r = calculate_angle([hip_r.x, hip_r.y], [knee_r.x, knee_r.y], [ankle_r.x, ankle_r.y])
        avg_knee_angle = (knee_angle_l + knee_angle_r) / 2

        back_angle_l = calculate_angle([shoulder_l.x, shoulder_l.y], [hip_l.x, hip_l.y], [knee_l.x, knee_l.y])
        back_angle_r = calculate_angle([shoulder_r.x, shoulder_r.y], [hip_r.x, hip_r.y], [knee_r.x, knee_r.y])
        avg_back_angle = (back_angle_l + back_angle_r) / 2

        if self.stage is None:
            if avg_knee_angle > 160 and avg_back_angle > 150:
                self.stage = "up"
            return self.counter, self.stage

        if avg_knee_angle < 120 and avg_back_angle < 110 and self.stage == "up":
            self.stage = "down"

        if avg_knee_angle > 160 and avg_back_angle > 150 and self.stage == "down":
            self.stage = "up"
            self.counter += 1
